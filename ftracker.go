...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	speed := meanSpeed(action, duration)
	calories := (runningCaloriesMeanSpeedMultiplier * runningCaloriesMeanSpeedShift * speed) * weight / mInKm * duration * minInH
	return calories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedMsec, 2)/heightMetr)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
